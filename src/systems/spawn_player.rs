...
pub fn spawn_player(mut commands: Commands, asset_server: Res<AssetServer>, config: Res<Config>) {
    let player = commands.spawn(PlayerBundle::new(&asset_server, config.scale)).id();
