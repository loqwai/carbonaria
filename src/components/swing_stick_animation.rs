...
pub struct SwingStickAnimation {
    ...
    pub reset_handle: Handle<AnimationClip>,
    pub duration: f32,
}
...
impl SwingStickAnimation {
    pub fn new(animations: &mut ResMut<Assets<AnimationClip>>, name: Name) -> SwingStickAnimation {
        SwingStickAnimation {
            handle: animations.add(build_animation(name.clone())),
            reset_handle: animations.add(build_reset_animation(name.clone())),
            duration: 0.2,
        }
    }
}
...
fn build_reset_animation(name: Name) -> AnimationClip {
    let mut animation: AnimationClip = Default::default();
